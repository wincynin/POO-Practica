...
public class CommandHandler {
    ...
    //Constructor
    public CommandHandler(Store store, Ticket ticket) {
        this.store = store;
        this.ticket = ticket;
    }
            ...
            case "add":
                ...
                if (comandosAdd.length >= 3) {
                    try {
                        ...
                        ProductCategory productCategory = ProductCategory.valueOf(comandosAdd[2].split(" ")[1].toUpperCase()); //Le asigna una categoria
                        double price = Double.parseDouble(comandosAdd[2].split(" ")[2]); //Convierte a un double=precio

                        Product prod = new Product(id, name, productCategory, price); //Crea el producto a partir del comando introducido
                ...
                if (comandosAdd.length >= 3) {
                    try {
                        ...
                        if (store.updateProduct(idProd, campo, actualizacion)) {
                            System.out.println(store.getProduct(idProd));
            ...
            case "add":
                if (comandoTicket.length >= 3) {
                    try {
                        ...
                        Product prod = store.getProduct(id);
